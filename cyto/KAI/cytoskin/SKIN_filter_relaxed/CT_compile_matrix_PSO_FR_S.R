...
for(i in 1:length(psoriasis)){  
  ...
  if(nrow(df)>0){
    ...
    #filter the data based on inter quartile rate
    if(nrow(dfr)>1){
      d=as.data.frame(dfr)
...
write.table(file="~/Documents/KAI/cytoskin/SKIN_filter_relaxed/data.psoriasis_FR.txt",DataPsoriasisSkin, sep="\t", quote=F )      
save(DataPsoriasisSkin, file="~/Documents/KAI/cytoskin/SKIN_filter_relaxed/data.psoriasis_FR.RData")
