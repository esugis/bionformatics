...
for(i in 1:length(vitiliigo1)){  
 ...
 # CTdata1[,2:17]=apply(CTdata1[,2:17],2, as.numeric)
  ...
  for (i in seq(1, length(df[,2]), 3)){  
    ...
    print("sum of nas")
    print(s)
    if (sum(is.na(expression)) < 3 ){#changed from 2 to 3
...
save(DataVitiligoBlood, file="~/Documents/KAI/cytoskin/BLOOD_filter_relaxed/datavitiligo.RData")
write.table(file="~/Documents/KAI/cytoskin/BLOOD_filter_relaxed/datavitiligo.txt",DataVitiligoBlood, sep="\t", quote=F )      
