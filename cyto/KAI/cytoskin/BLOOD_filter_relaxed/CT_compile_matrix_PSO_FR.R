...
for(i in 1:length(psoriasis1)){  
 ...
 # CTdata1[,2:17]=apply(CTdata1[,2:17],2, as.numeric)
  ...
  for (i in seq(1, length(df[,2]), 3)){  
    ...
    print(s)
    if (sum(is.na(expression)) < 3 ){# changed 2 to 3 samples
...
save(DataPsoriasisBlood_QR, file="~/Documents/KAI/cytoskin/BLOOD_filter_relaxed/datapsoriasis_QR.RData")
write.table(file="~/Documents/KAI/cytoskin/BLOOD_filter_relaxed/datapsoriasis_QR.txt",DataPsoriasisBlood_QR, sep="\t", quote=F )      
