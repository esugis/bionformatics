...
#read in row names
CTdata1=read.csv(file=vitiliigo1[1], na.strings = "")
CTdata1=CTdata1[7:105,2:16]
CTdata2=read.csv(file=vitiliigo2[1], na.strings = "")
CTdata2=CTdata2[7:69,2:16]
...
length(rowsC)
#[1] 105
DataVitiligoBlood=rowsC
DataVitiligoBlood=as.data.frame(DataVitiligoBlood)
colnames(DataVitiligoBlood)=c("SAMPLE")
library(functional)
#for each of controlls
#selected.patient=c()
#selected.expression=c()
for(i in 1:length(vitiliigo1)){  
  #for(i in 3:5){
  f1=vitiliigo1[i] 
  f2=vitiliigo2[i]
  #f1=vitiliigo1[8]
 ...
 # CTdata1[,2:17]=apply(CTdata1[,2:17],2, as.numeric)
  ...
  if(nrow(df)>0){
    ...
    if(nrow(dfr)>1){
      ##using MAD
      filt=median(as.numeric(dfr[,8]),na.rm=T)+2*mad(as.numeric(dfr[,8]),na.rm=T)#filter based on the reference gene cycles
      dfr.f=dfr[dfr[,8]<=filt,]#keep samples with reference genes number of cycles< meadian+2mad
      #print("outliers")
      #print(dfr[dfr[,8]>filt,])
  ...
  #print("new geneEXP")
  #print(geneExpr)
  DataVitiligoBlood=merge(DataVitiligoBlood,geneExpr, by.x="SAMPLE", by.y="SAMPLE", all.x=T )
  #print(DataVitiligoBlood)
}
##using MAD
DataVitiligoBlood=DataVitiligoBlood
save(DataVitiligoBlood, file="~/Documents/KAI/cytoskin/BLOOD/datavitiligo.RData")
write.table(file="~/Documents/KAI/cytoskin/BLOOD/datavitiligo.txt",DataVitiligoBlood, sep="\t", quote=F )      

##using QR
#DataVitiligoBlood_QR=DataVitiligoBlood
#save(DataVitiligoBlood_QR, file="~/Documents/KAI/cytoskin/BLOOD/datavitiligo_QR.RData")
#write.table(file="~/Documents/KAI/cytoskin/BLOOD/datavitiligo_QR.txt",DataVitiligoBlood_QR, sep="\t", quote=F )      

###Choose outliers filtering method
#median +- 2*Median Absolute Deviations
filt=median(as.numeric(dfr[,8]),na.rm=T)+2*mad(as.numeric(dfr[,8]),na.rm=T)
dfr.f=dfr[dfr[,8]<filt,]
dim(dfr.f)
#[1] 69 17
dim(dfr)
#[1] 75 17
...
dfr.f=as.data.frame(dfr.f)
dfr.f[,8]=as.numeric(as.character(dfr.f[, 8]))
d=as.data.frame(dfr)
d[,8]=as.numeric(as.character(d[, 8]))
dn <- density(d[, 8])
plot(dn, main="distribution")
ggplot(d, aes(x=CT.1)) +
  theme_bw()+
  geom_density(alpha=.2, fill="#FF6666")

##based on the upper and lower quartiles of the data distribution;
#interquartile distance, a measure of the spread of the data similar to the standard deviation
#commonly used in this outlier detection rule is c = 1.5
#because it does not depend on an estimate of the “center” of the data 
#(e.g., the mean in the ESD identifier or the median in the Hampel identifier),
#it is better suited to distributions that are moderately asymmetric.
boxplot(d[,8])#remove one outlier
z <- d[d[,8] > quantile(d[,8], .25) - 1.5*IQR(d[,8]) & d[,8] < quantile(d[,8], .75) + 1.5*IQR(d[,8]),]
ggplot(z,aes(x=CT.1)) +
  theme_bw()+
  geom_density(alpha=.2, fill="#FF6666")
