...
for(i in 1:length(psoriasis1)){  
 ...
 # CTdata1[,2:17]=apply(CTdata1[,2:17],2, as.numeric)
  ...
  if(nrow(df)>0){
    ...
    dfr=t(dfr[c(F,F,T)])####????????do i need to remove that?seems like an error
  ...
  #print("new geneEXP")
  #print(geneExpr)
  DataPsoriasisBlood=merge(DataPsoriasisBlood,geneExpr, by.x="SAMPLE", by.y="SAMPLE", all.x=T )
  #print(DataControllsBlood)
}
##using MAD
#DataPsoriasisBlood=DataPsoriasisBlood
#save(DataPsoriasisBlood, file="~/Documents/KAI/cytoskin/BLOOD/datapsoriasis.RData")
#write.table(file="~/Documents/KAI/cytoskin/BLOOD/datapsoriasis.txt",DataPsoriasisBlood, sep="\t", quote=F )      

##alternatively filter on quartiles
DataPsoriasisBlood_QR=DataPsoriasisBlood
save(DataPsoriasisBlood_QR, file="~/Documents/KAI/cytoskin/BLOOD/datapsoriasis_QR.RData")
write.table(file="~/Documents/KAI/cytoskin/BLOOD/datapsoriasis_QR.txt",DataPsoriasisBlood_QR, sep="\t", quote=F )      
