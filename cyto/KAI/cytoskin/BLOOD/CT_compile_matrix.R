...
filt=median(as.numeric(dfr[,6]),na.rm=T)+2*mad(as.numeric(dfr[,6]),na.rm=T)
dfr.f=dfr[dfr[,6]<filt,]
dim(dfr.f)
#[1] 69 17
dim(dfr)
#[1] 75 17
...
dfr.f=as.data.frame(dfr.f)
dfr.f[,6]=as.numeric(as.character(dfr.f[, 6]))
d=as.data.frame(dfr)
...
boxplot(d[,6])#remove one outlier
z <- d[d[,6] > quantile(d[,6], .25) - 1.5*IQR(d[,6]) & d[,6] < quantile(d[,6], .75) + 1.5*IQR(d[,6]),]
ggplot(z,aes(x=CT.1)) +
  theme_bw()+
  geom_density(alpha=.2, fill="#FF6666")
