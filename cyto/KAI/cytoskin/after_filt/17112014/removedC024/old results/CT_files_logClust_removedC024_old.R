...
pheatmap(data.pca, cluster_rows = F, cluster_cols = F,scale = "none", main="Data log-transformed, contains NA",
...
main="Filtered data after imputation. KNN, k=4", filename="~/Documents/KAI/cytoskin/after_filt/17112014/removedC024/filtered_imputed_knn4.png")
...
library(amap)
s=set.seed(1234)
km=Kmeans(data.pca.imp,6,iter.max = 100500, nstart =50,
          method = "correlation")
head(km)
...
Groups=str_replace(Groups,"PH.*","PS")#sick
Groups=str_replace(Groups,"PT.*","PH")#healthy
Groups=str_replace(Groups,"VH.*","VS")#sick
Groups=str_replace(Groups,"VT.*","VH")#healthy
...
rownames(data.pca.knn)=str_replace(rownames(data.pca.knn),"PH","PS")#sick
rownames(data.pca.knn)=str_replace(rownames(data.pca.knn),"PT","PH")#healthy
rownames(data.pca.knn)=str_replace(rownames(data.pca.knn),"VH","VS")#sick
rownames(data.pca.knn)=str_replace(rownames(data.pca.knn),"VT","VH")#healthy
