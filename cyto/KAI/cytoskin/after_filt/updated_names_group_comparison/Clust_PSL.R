#This script performes separate analysis of genes, expressed in PSL samples.
#This is done separtely because some of the gene were not expressed in othre samples.
#Script
#-Reads the files,
...
######not fun part
psl1=psl[,! apply( psl , 2 , function(x) all(is.na(x)) ) ]#removes columns with complete NAs
pheatmap(psl1, cluster_rows = F, cluster_cols = F,scale = "none", main="data")#data.pca=data.pca[!rowSums(!is.finite(data.pca)),]#remove nas

# remove rows with more than 50 NAs
numNAs <- apply(psl1, 1, function(z) sum(is.na(z)))
psl.new=psl1[!(numNAs > 0.5*length(colnames(psl1))),]
pheatmap(psl.new, cluster_rows = F, cluster_cols = F, scale = "row", main="data")
dim(psl.new) #[1] 41 33
#remove cols with more than 50% NAs
psl=t(psl.new)
numNAs <- apply(psl, 1, function(z) sum(is.na(z)))
psl.new=psl[!(numNAs >0.5*length(colnames(psl))),]#previously it was 9
psl=t(psl.new)
pheatmap(psl, cluster_rows = F, cluster_cols = F,scale = "none",main="Gene expression in psoriatic lesional skin samples.",
         filename="psl_log_filt_NAs.png")

dim(psl)#41 32
psl_log2_filt=psl
save(psl_log2_filt,file = "~/Documents/KAI/cytoskin/after_filt/updated_names_group_comparison/psl_log2_filt.RData")


#Impute the missing data using k nearest neighbours all data together
library(impute)
library(RColorBrewer)
psl.imp=impute.knn(psl ,k = 4, rowmax = 0.5, colmax = 0.8, maxp = 1500, rng.seed=36243606)
save(psl.imp,file="imputed_psl_knn4.RData")

psl.imp=as.data.frame(psl.imp$data)
pheatmap(psl.imp, cluster_rows = T, cluster_cols = F,clustering_distance_rows = "correlation",
...
main="Gene expression in PL samples after imputation", filename="filtered_imputed_knn4.png")
...
#take 4 clusters as it shows good results


library(amap)

hc.data.psl=hcluster(psl.imp, method = "correlation", diag = FALSE, upper = FALSE,
                 link = "complete", members = NULL, nbproc = 2,
                 doubleprecision = TRUE)

plot(hc.data.psl)
ct=cutree(hc.data.psl, k=6)
save(hc.data.psl,ct,file="hclust_psl.RData")
sort(ct)
...
psl.imp=cbind(rownames(psl.imp), psl.imp)
colnames(psl.imp)[1]="rn"
data_clust1 <- cbind(psl.imp[psl.imp$rn%in%hc.cl.1,],1)#add column cluster number
data_clust1=data_clust1[, c(-1)]
colnames(data_clust1)[33]="Cluster"
data_clust2 <- cbind(psl.imp[psl.imp$rn%in%hc.cl.2,],2)
data_clust2=data_clust2[, c(-1)]
colnames(data_clust2)[33]="Cluster"
data_clust3 <- cbind(psl.imp[psl.imp$rn%in%hc.cl.3,],3)
data_clust3=data_clust3[, c(-1)]
colnames(data_clust3)[33]="Cluster"
data_clust4 <- cbind(psl.imp[psl.imp$rn%in%hc.cl.4,],4)
data_clust4=data_clust4[, c(-1)]
colnames(data_clust4)[33]="Cluster"
data_clust5 <- cbind(psl.imp[psl.imp$rn%in%hc.cl.5,],5)
data_clust5=data_clust5[, c(-1)]
colnames(data_clust5)[33]="Cluster"
data_clust6 <- cbind(psl.imp[psl.imp$rn%in%hc.cl.6,],6)
data_clust6=data_clust6[, c(-1)]
colnames(data_clust6)[33]="Cluster"
data_clust=rbind(data_clust1,data_clust2,data_clust3,data_clust4,data_clust5,data_clust6)
o<- order(data_clust[,33])###sort by cluster number
data_clust <- data_clust[o, ]###sort by cluster number
head(data_clust)
data_hc=data_clust
library(pheatmap)
library(plyr)
colnames(data_clust)[33]="Cluster"###add column name
#Go summaries
library(stringr)
...
annot=as.data.frame(annotation[1:32,1])
colnames(annot)="Groups"

Groups=Groups[1:32]
data_clust=data_clust[,1:32]
...
plot(gs_exp, fontsize = 8, classes = "Groups", term_length=150, filename = "Gosummaries_psl_log2_knn4_hc.pdf")

#save(ct,hc.data,data_hc, file = "~/Documents/KAI/cytoskin/after_filt/updated_names_group_comparison/hc.RData")

#find significant clusters
library(pvclust)
result.psl=pvclust(t(psl.imp[,2:33]), method.hclust="complete",
               method.dist="correlation",
               nboot=1000, r=seq(.5,1.4,by=.1), store=FALSE, weight=FALSE)
plot(result.psl)
pvrect(result.psl, alpha=0.95)
save(result.psl,file="pvclust_psl.RData")
...
library(amap)
s=set.seed(1234)
km.psl=Kmeans(psl.imp,6,iter.max = 100500, nstart =100,
          method = "correlation")
# if needed to compare load(file = "~/Documents/KAI/cytoskin/after_filt/17112014/removedC024/km6.RData")
head(km.psl)
save(km.psl, file="kmeans6_psl.RData")
data_clust <- cbind(psl.imp,km.psl$cluster)#add column cluster number
o<- order(data_clust[,33])###sort by cluster number
data_clust <- data_clust[o, ]###sort by cluster number
head(data_clust)
data_km=data_clust
...
plot(gs_exp, fontsize = 10, term_length=50, classes = "Groups", filename = "~/Documents/KAI/cytoskin/after_filt/updated_names_group_comparison/Gosummaries_psl_kmean6_knn4.pdf")
##end go summaries
...
save(pca.all,Groups,Samples,file="~/Documents/KAI/cytoskin/after_filt/updated_names_group_comparison/pca.RData")
#save(data.log,Groups,Samples,file="~/Documents/KAI/cytoskin/after_filt/updated_names_group_comparison/pca.RData")

 ##Plot without lables

a=qplot(pca.all$x[,1],pca.all$x[,2],xlab="Principal Component 1",ylab= "Principal Component 2",  colour = Groups)+scale_colour_manual(values=c("black","darkblue","seagreen3","red","darkorchid1"))
a1=a+theme(legend.position="none")+ stat_ellipse()
...
b1=b+theme(legend.position="none")+ stat_ellipse()
...
c1=c + theme(legend.position="none")+ stat_ellipse()
...
w1=qplot(y, x,data=dd1, geom="bar",fill = I("grey50"),width=.5, stat="identity") + coord_flip()+labs(y="Rotation matrix weights for the first principal component", x="Gene names")+theme (axis.text.y = element_text(colour="grey20",size=8,face="plain"),axis.title.x = element_text(size=10,face="plain"))
...
w2=qplot(y, x,data=dd2, geom="bar",fill = I("grey50"),width=.5, stat="identity") + coord_flip()+labs(y="Rotation matrix weights for the second principal component", x="Gene names")+theme (axis.text.y = element_text(colour="grey20",size=8,face="plain"),axis.title.x = element_text(size=10,face="plain"))
...
w3=qplot(y, x,data=dd3, geom="bar",fill = I("grey50"),width=.5, stat="identity") + coord_flip()+labs(y="Rotation matrix weights for the third principal component", x="Gene names")+theme(axis.text.y = element_text(colour="grey20",size=8,face="plain"),axis.title.x = element_text(size=10,face="plain"))
