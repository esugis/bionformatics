...
################################################ Disease onset ##################
###peredelat iz vitiliigo, Ne nashla original 
...
save(pso_fam_groups_comp, file = "~/Documents/KAI/cytoskin/after_filt/21112014/Psoriasis_in_family_group_comparison.RData")
...
ggplot(outDf, aes(x=diff,y=groups,color=p_adj))+
  ...
  ggtitle("Comparison of groups using log-transformed data.
          Groups are formed based on the combinations of factors (Skin:PH/PT,Psoriasis in the family: yes/no")
...
save(pso_arthritis_groups_comp, file = "~/Documents/KAI/cytoskin/after_filt/21112014/Psoriatic_arthritis_in_family_group_comparison.RData")
...
ggplot(outDf, aes(x=diff,y=groups,color=p_adj))+
  ...
  geom_errorbarh(aes(xmax = upr, xmin = lwr, height = .2))+
  ggtitle("Comparison of groups using log-transformed data.
          Groups are formed based on the combinations of factors (Skin:PH/PT,Psoriatic arthritis: yes/no")
...
save(pso_nail_inv_groups_comp, file = "~/Documents/KAI/cytoskin/after_filt/21112014/Psoriasisc_nail_involvement_group_comparison.RData")
...
ggplot(outDf, aes(x=diff,y=groups,color=p_adj))+
  ...
  geom_errorbarh(aes(xmax = upr, xmin = lwr, height = .2))+
  ggtitle("Comparison of groups using log-transformed data.
          Groups are formed based on the combinations of factors (Skin:PH/PT,Nail involvement: yes/no")
...
save(pso_pasi_groups_comp, file = "~/Documents/KAI/cytoskin/after_filt/21112014/Psoriasis_PASI_group_comparison.RData")
...
ggplot(outDf, aes(x=diff,y=groups,color=p_adj))+
  ...
  geom_errorbarh(aes(xmax = upr, xmin = lwr, height = .2))+
  ggtitle("Comparison of groups using log-transformed data.
          Groups are formed based on the combinations of factors (Skin:PH/PT,PASI score: (0,10]/(10,20]/(20,Inf]")
...
save(pso_dis_onset_pso_fam_comp, file = "~/Documents/KAI/cytoskin/after_filt/21112014/Psoriasis_dis_onset_pso_fam_group_comparison.RData")
...
ggplot(outDf, aes(x=diff,y=groups,color=p_adj))+
  ...
  geom_errorbarh(aes(xmax = upr, xmin = lwr, height = .2))+
  ggtitle("Comparison of groups using log-transformed data.
          Groups are formed based on the combinations of factors (Skin:PH/PT,Disease onset:old(>40), young(<40), Psoriasis in family: yes/no ")
...
save(pso_dis_onset_pasi_comp, file = "~/Documents/KAI/cytoskin/after_filt/21112014/Psoriasis_dis_onset_pasi_group_comparison.RData")
...
ggplot(outDf, aes(x=diff,y=groups,color=p_adj))+
 ...
 # theme(axis.text.x = element_text(colour="black"), axis.text.y = element_text(size=6))+
  geom_errorbarh(aes(xmax = upr, xmin = lwr, height = .2))+
  ggtitle("Comparison of groups using log-transformed data.
          Groups are formed based on the combinations of factors (Skin:PH/PT,Disease onset:old(>40), young(<40),PASI score: (0,10]/(10,20]/(20,Inf]")
ggsave('~/Documents/KAI/cytoskin/after_filt/21112014/pso_phpt_dis_onset_pasi.png', height = 20, width = 11)
...
save(pso_skin_phototype_comp, file = "~/Documents/KAI/cytoskin/after_filt/21112014/Psoriasis_pso_skin_phototype_group_comparison.RData")
...
ggplot(outDf, aes(x=diff,y=groups,color=p_adj))+
  ...
  geom_errorbarh(aes(xmax = upr, xmin = lwr, height = .2))+
  ggtitle("Comparison of groups using log-transformed data.
          Groups are formed based on the combinations of factors (Skin:PH/PT, Skin phototype: I/II/III")

###############################Duration of the  psoriasis ##################

annotation.h=annotationh[rownames(annotationh)%in%colnames(p)[1:34],colnames(annotationh)%in%c("Gender","Duration_of_disease(years)")]
annotation.h[,2]=str_replace(annotation.h[,2],",",".")
annotation.h=data.frame(annotation.h)
annotation.h[,2]=as.numeric(as.vector(annotation.h[,2]))
annotation.h[,2]=cut(annotation.h[,2], c(0, 5, 15, Inf))

annotation.t=annotationt[rownames(annotationt)%in%colnames(p)[35:69],colnames(annotationt)%in%c("Gender","Duration_of_disease(years)")]
annotation.t[,2]=str_replace(annotation.t[,2],",",".")
annotation.t=data.frame(annotation.t)
annotation.t[,2]=as.numeric(as.vector(annotation.t[,2]))
...
annotation.h=annotationh[rownames(annotationh)%in%colnames(p)[1:34],colnames(annotationh)%in%c("Age_of_disease_onset", "Nail_involvment")]
annotation.h=annotation.h[order(annotation.h[,1]),]
annotation.h[annotation.h[,1]>=40, 1]<-"old"
annotation.h[annotation.h[,1]< 40,1]<-"young"
annotation.t=annotationt[rownames(annotationt)%in%colnames(p)[35:69],colnames(annotationt)%in%c("Age_of_disease_onset", "Nail_involvment")]
annotation.t=annotation.t[order(annotation.t[,1]),]
annotation.t[annotation.t[,1]>=40, 1]<-"old"
annotation.t[annotation.t[,1]<40, 1]<-"young"
...
save(pso_dis_onset_nail_inv_comp, file = "~/Documents/KAI/cytoskin/after_filt/21112014/Psoriasis_dis_onset_nail_inv_comp_group_comparison.RData")
...
ggsave('~/Documents/KAI/cytoskin/after_filt/21112014/pso_dis_onset_nail_inv_log.png', height = 20, width = 11)
