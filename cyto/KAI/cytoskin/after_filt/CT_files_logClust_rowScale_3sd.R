...
if(nrow(df)>0){
  print("nrows > 0")
    ...
    dfr=t(dfr[c(F,F,T)])
     ...
     #filter the data based on mean +3 std
     if(nrow(dfr)>1){
       filt=median(as.numeric(dfr[,6]),na.rm=T)+3*sd(as.numeric(dfr[,6]),na.rm=T)#filter based on the reference gene cycles
...
#write.table(file="/data.control_newfilt.txt",DataControllsSkin, sep="\t", quote=F )      
...
data.pca_3SD=t(data.pca.new)
pheatmap(data.pca_3SD, cluster_rows = F, cluster_cols = F,scale = "none",main="All data filtered based on 3SD (size 35x122)")
         
dim(data.pca_3SD)#35 122
data_log2_filt_3SD=data.pca_3SD
colnames(data_log2_filt_3SD)=str_replace(colnames(data_log2_filt_3SD),"PH","PSL")#sick
colnames(data_log2_filt_3SD)=str_replace(colnames(data_log2_filt_3SD),"PT","PSNL")#healthy
colnames(data_log2_filt_3SD)=str_replace(colnames(data_log2_filt_3SD),"VH","VL")#sick
colnames(data_log2_filt_3SD)=str_replace(colnames(data_log2_filt_3SD),"VT","VNL")#healthy


save(data_log2_filt_3SD, file = "data_log2_filt3SD.RData")
