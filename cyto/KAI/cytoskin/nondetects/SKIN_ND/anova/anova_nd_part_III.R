...
annotation.pso.h.t=data.frame(cbind(Sample,annotation.pso.h.t))
annotation.pso.h.t=cbind(annotation.pso.h.t,interactions=interaction(annotation.pso.h.t$Sample, annotation.pso.h.t$Age_of_disease_onset, annotation.pso.h.t$Psoriasis_in_family))
...
ggsave("ANOVA_PS_typeI_typeII_PL_PN.png", width=15, height=10)
...
annotation.pso.h.t=cbind(annotation.pso.h.t, interactions=interaction(annotation.pso.h.t$Age_of_disease_onset, annotation.pso.h.t$PASI))
ann=cbind(annotation.pso.h.t,sample= rownames(annotation.pso.h.t))
...
#95% confidence intervals visualizing the results of pairwise comparisons of the mean levels of gene expression
#grouped by skin state: lesional(PL)/nonlesional(PNL),age of disease onset: <40(early),>40(late), and PASI score: low (0,10], medium (10,20], high (20,Inf].
#The intervals that do not overlap with zero point correspond to statistically significant differences at P < 0.05.
...
ggplot(outDf, aes(x=Difference,y=Groups,color=p_adj))+
  ...
  theme(axis.text.x = element_text(colour="black"), axis.text.y = element_text(size=9))+
  geom_errorbarh(aes(xmax = upr, xmin = lwr, height = .2))
ggsave("ANOVA_ps_early.severe_late.mild.png", width=15, height=10)
...
annotation.pso.h.t[,2]=as.character(annotation.pso.h.t[,2])
annotation.pso.h.t[,3]=as.character(annotation.pso.h.t[,3])
annotation.pso.h.t=cbind(annotation.pso.h.t, interactions=interaction(annotation.pso.h.t$Sample, annotation.pso.h.t$Age_of_disease_onset, annotation.pso.h.t$PASI))
...
ggplot(tmp, aes(x=Gene,y=Expression,color=interactions))+
  geom_jitter(aes(shape = interactions),position = position_jitter(width = .21),size = 2.5)+
  theme_bw()
ggsave("Gene_exp_PL_PNL_early_late_severe.png", width=20, height=10)
#Aov example for many genes
myAov <- function(df){
  aov_list <- aov(Expression~interactions, data=df)
  aov_out <- as.data.frame(TukeyHSD(aov_list, "interactions")$interactions)
...
ggplot(PL_PNL_early_late_severe, aes(x=Difference,y=Groups,color=p_adj))+
  ...
  theme(axis.text.x = element_text(colour="black"), axis.text.y = element_text(size=9))+
  geom_errorbarh(aes(xmax = upr, xmin = lwr, height = .2))
ggsave("PL_PNL_early_late_severe.png", width=12, height=10, dpi=100)
