...
#PSO
pso=t(DDCTBloodB2mgF[71:173,])# psoriasis
rows=rownames(pso)
cols=colnames(pso)
...
ggplot(dat.t[1:35,], aes(x=PL, y=PMA)) + geom_point(shape=1) +geom_smooth(method=lm)+
  theme_bw()+
  geom_text(data=data.frame(correlations.adj.pval.sign[3,]), aes(label=paste("r=", round(r,5), sep="")), x=1, y=-0.25)+
  geom_text(data=data.frame(correlations.adj.pval.sign[3,]), aes(label=paste("pval=", round(adj.pval,5), sep="")), x=1, y=-0.5)+
...
ggplot(dat.t[1:35,], aes(x=SEB, y=PMA)) + geom_point(shape=1) +geom_smooth(method=lm)+
  theme_bw()+
  geom_text(data=data.frame(correlations.adj.pval.sign[4,]), aes(label=paste("r=", round(r,5), sep="")), x=1, y=2)+
  geom_text(data=data.frame(correlations.adj.pval.sign[4,]), aes(label=paste("pval=", round(adj.pval,5), sep="")), x=1, y=1.75)+
  ggtitle("Correlation between expression profiles of CTLA4 gene \n in groups SEB and PMA") 

ggplot(dat.t[1:35,], aes(x=SEB, y=Unstimulated)) + geom_point(shape=1) +geom_smooth(method=lm)+
  theme_bw()+
  geom_text(data=data.frame(correlations.adj.pval.sign[5,]), aes(label=paste("r=", round(r,5), sep="")), x=1, y=2)+
  geom_text(data=data.frame(correlations.adj.pval.sign[5,]), aes(label=paste("pval=", round(adj.pval,5), sep="")), x=1, y=1.75)+
  ggtitle("Correlation between expression profiles of CTLA4 gene \n in groups SEB and Unstimulated") 
...
#2 select results of #1 from skin_blood_group
