...
length(rowsC)
#[1] 75
DataControllsBloodBact=rowsC
DataControllsBloodBact=as.data.frame(DataControllsBloodBact)
colnames(DataControllsBloodBact)=c("SAMPLE")
library(functional)
...
for(i in 1:length(controlls1)){  
 ...
 # CTdata1[,2:17]=apply(CTdata1[,2:17],2, as.numeric)
  ...
  #print("new geneEXP")
  #print(geneExpr)
  DataControllsBloodBact=merge(DataControllsBloodBact,geneExpr, by.x="SAMPLE", by.y="SAMPLE", all.x=T )
  #print(DataControllsBloodBact)
}
##using MAD
#DataControllsBloodBact=DataControllsBloodBact[c(1:48,52:75),]
#save(DataControllsBloodBact, file="~/Documents/KAI/cytoskin/BLOOD/DataControllsBloodBact.RData")
#write.table(file="~/Documents/KAI/cytoskin/BLOOD/DataControllsBloodBact.txt",DataControllsBloodBact, sep="\t", quote=F )      
##using Quqrtiles
DataControllsBloodBact_QR=DataControllsBloodBact[c(1:48,52:75),]
dim(DataControllsBloodBact_QR)
save(DataControllsBloodBact_QR, file="~/Documents/KAI/cytoskin/BLOOD_BACT/DataControllsBloodBact_QR.RData")
write.table(file="~/Documents/KAI/cytoskin/BLOOD_BACT/DataControllsBloodBact_QR.txt",DataControllsBloodBact_QR, sep="\t", quote=F )      
...
dim(dfr.f.med)#56 and in boxplot 62
...
library(ggplot2)
boxplot(d[,8], main="Number of cycles of reference gene", 
        xlab="Gene IL17F", ylab="Ct")#remove one outlier
